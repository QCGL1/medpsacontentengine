...
class Config:
    ...
    def load(cls) -> "Config":
        ...
        # In dry-run we don't require real keys, so Sahil can test end-to-end
        # without any paid credentials.
        req = not dry
        return cls(
            meta_access_token=_get("META_ACCESS_TOKEN", required=req),
            meta_page_id=_get("META_PAGE_ID", required=req),
            meta_ig_account_id=_get("META_IG_ACCOUNT_ID", required=req),
